...
const InProgressScreen = () => {
  ...
  const inProgressTasks = useMemo(() => {
    return data?.filter((task) => task.status === 'IN_PROGRESS') ?? [];
  }, [data]);
  ...
  const handleCreateTask = () => {
    ...
    // Convert to UTC with time set to 00:00:00.000Z
      ...
      <Modal
        ...
        <Pressable
          ...
          <Pressable className="m-8 w-72 rounded-xl bg-white" onPress={(e) => e.stopPropagation()}>
            ...
            {showStatusDropdown &&
              getStatusOptions(
                inProgressTasks.find((task) => task.id === showStatusDropdown)?.status || ''
              ).map((status) => (
                <Pressable
                  key={status}
